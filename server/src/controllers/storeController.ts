...
export const createStore = async (request: AuthenticatedRequest, reply: FastifyReply) => {
  ...
  const { name, description, address, city, postalCode, phone, email } = request.body as Store;

  try {
    const creator = await AppDataSource.getRepository(User).findOne({ where: { id: userId } });
