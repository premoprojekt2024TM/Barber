...
const HairdresserPage = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance1.get('/api/hairdresser', { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
        if (response.data.isValid) {
          if (response.data.role !== 'hairdresser') {
            setMessage('You do not have permission to access this page.');
            setTimeout(() => navigate('/auth/sign-in'), 3000);
          } else {
            setMessage('Welcome to the Hairdresser Page!');
          }
        } else {
